...
import org.wallride.admin.service.CategoryService;
...
public class ArticleCreateController {
	...
	public String draft(
		...
		try {
			article = articleService.createArticle(form, errors, Post.Status.DRAFT, authorizedUser);
		}
	...
	public String publish(
		...
		Article article = null;
		try {
			article = articleService.createArticle(form, errors, Post.Status.PUBLISHED, authorizedUser);
		}
		catch (BindException e) {
			if (errors.hasErrors()) {
				logger.debug("Errors: {}", errors);
